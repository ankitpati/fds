...
mat mat_alloc(unsigned r, unsigned c)
{
    unsigned i;
    mat m;
    if(!(m.m=malloc(sizeof(*m.m)*r))){
        m.r=m.c=0;
        return m;
    }
    for(i=0; i<r; ++i)
        if(!(m.m[i]=malloc(sizeof(**m.m)*c))){
            for(r=0; r<i; ++r) free(m.m[r]);
            free(m.m);
            m.m=NULL;
            m.r=m.c=0;
            return m;
...
mat mat_cpy(mat m)
{
    unsigned i, j;
    mat t;
    if(!(t=mat_alloc(m.r, m.c)).m) return t;
    t.r=m.r;
    t.c=m.c;
    for(i=0; i<m.r; ++i)
        for(j=0; j<m.c; ++j)
...
mat mat_mul(mat m1, mat m2)
{
    ...
    if(m1.c!=m2.r){
        ...
        if(!(t=mat_alloc(m1.r, m2.c)).m) return t;
        for(i=0; i<m1.r; ++i)
            for(j=0; j<m2.c; ++j)
                for(k=t.m[i][j]=0; k<m2.r; ++k)
